...
export function ExpenseTrxForm({
  onSubmit,
  onCancel,
  initial,
  loading,
  ...
  const initialAssetAccountId =
    initial?.items.find((item) => item.credit > 0)?.accountId ?? "";
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const expenseChild = accounts.find(
      (a) =>
    ...
    if (!expenseChild) {
      setFormError("No expense child account found for selected asset");
      return;
    }

    const items = [
      { accountId: expenseChild.id, debit: numAmount, credit: 0 },
